...
# MEGA SDK v10.19.0 no longer exposes the short startUpload(path,parent,name,listener)
# or startDownload(node,path,listener) overloads. Use the still-supported full transfer
# overloads so the reflection bridge matches the actual official Java API at runtime.
manager = ROOT / "app/src/main/java/jp/linkserver/nittcsc/sync/CloudFileSyncManager.kt"
replace_once(
    manager,
...
print("sync labels and MEGA API compatibility patched")
